...
if(isset($_SESSION["tourist_loggedin"]) && $_SESSION["tourist_loggedin"] === true){
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    {
        $username = trim($_POST["v_email"]);
    }
    
    // Check if password is empty
    if(empty(trim($_POST["v_password"])))
    {
    ...
    if(empty($username_err) && empty($password_err))
    {
        ...
        $login_sql = "SELECT * FROM visa WHERE v_email = '$username' and v_password = '$enc_password1'";
        ...
        if($count == 1)
		{
			...
			$_SESSION['tourist_login'] = $username;
			header("location: index.php");
		}
		else
		{
...
<html>
   <head>
      ...
      <link href="https://fonts.googleapis.com/css?family=Montserrat:600,700,800,900,900i&display=swap" rel="stylesheet">
         ...
         <div class="main1">
               ...
               <div class="pure-g contain" >
                  <form enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="pure-form pure-form-aligned p4" style="display: inline-block; ">
                    <div class="pure-control-group <?php echo (!empty($username_err)) ? 'has-error' : ''; ?>">
                        ...
                        <input id="email" type="text" name="v_email"  value="<?php echo $username; ?>" placeholder="" class="pure-u-2-4 ps" required>
                        <span class="help-block"><?php echo $username_err; ?></span> 
                    </div>
                    <div class="pure-control-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
                        <label for="password">Password</label>
                        <input id="name" name="v_password" type="password" value="<?php echo $password; ?>" placeholder="" class="pure-u-2-4 ps" required>
            ...
            <div class="bg1 hd ">
               <div class="vs-inner pure-g" >
                  <div class="pure-u-1-1 p2">CUSTOMER</div>
