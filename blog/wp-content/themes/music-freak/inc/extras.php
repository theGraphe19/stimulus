 ...
 * @return array
 */
function music_freak_body_classes( $classes ) {
	...
	$classes[] = ! empty( $options['theme_color'] ) ? $options['theme_color'] : 'dark-version';
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'music-freak-selected-sidebar', true );
...
add_filter( 'body_class', 'music_freak_body_classes' );
