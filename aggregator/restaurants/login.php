<?php
   ...
   if(isset($_SESSION["h_loggedin"]) && $_SESSION["h_loggedin"] === true)
   {
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST")
   {
      ...
      if(empty(trim($_POST["h_password"])))
      {
         $password_err = "Please enter your password.";
      }
      else
      {
         $password = trim($_POST["h_password"]);
      }
      ...
      if(empty($email_err) && empty($password_err))
      {
         ...
         $login_sql = "SELECT * FROM hotels WHERE h_email = '$email' and h_password = '$enc_password1'";
               ...
               <div class="pure-g contain" >
                  <form enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="pure-form pure-form-aligned p4" style="display: inline-block; ">
                    ...
                    <div class="pure-control-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
                        ...
                        <input id="name" name="h_password" type="password" value="<?php echo $password; ?>" placeholder="" class="pure-u-2-4 ps" required>
